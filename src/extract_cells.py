...
def extract_cells(vidfile, maskfile, show_video = False):
    ...
    for frameNum, frame in enumerate(vf):#while( vf.isOpened() ):
        ...
        for i in range(number_of_segments):
            ...
            temp = cv2.morphologyEx( ims[i], cv2.MORPH_OPEN, kernel, iterations = 3)
        ...
        if(frameNum%100 == 1 or frameNum <20): #every 100 frames do full pairwise comparisons and save which were adjacent
            for i in range (5):
                del comparisons[:]
        ...
        for conts in range (number_of_segments): #bulds the contours and draws them onto the frame
            ...
            contours.append(cv2.findContours( dilates[conts], cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[0])
...
if __name__ == '__main__':
    ...
    #Optional args
    parser.add_argument("-o","--output", default = os.path.join(cwd, "single_cell_videos"),
    ...
    out = extract_cells(vidfile = args['input'], maskfile = args['masks'], show_video = args['showvid'])
